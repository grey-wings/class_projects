...
class TestOptions(BaseOptions):
    ...
    def __init__(self):
        ...
        parser = self.parser
        # parser.add_argument('--train_config', type=argparse.FileType(mode='r'), required=True,
        #                     help='config file saved from model training')
        parser.add_argument('--train_config', type=argparse.FileType(mode='r'),
                            default='/home/PointCloud/patch-forensics/checkpoints/pf220421-11/opt.yml',
                            help='config file saved from model training')
        # parser.add_argument('--train_config', type=argparse.FileType(mode='r'),
        #                     default='E:\\SME\\Data_Science\\AI_challenging_class_peojects\\fake_face_de'
        #                             'tection_220418\\patch-forensics\\checkpoints\\pf220418-16\\opt.yml',
        #                     help='config file saved from model training')
        parser.add_argument('--partition', type=str, default='test', help='val or test')
        # parser.add_argument('--dataset_name', type=str, required=True,
        #                     help="name to describe test dataset when saving results, e.g. celebahq_pgan")
        parser.add_argument('--dataset_name', type=str, default='kaggle2_bestval',
                            help="name to describe test dataset when saving results, e.g. celebahq_pgan")
        parser.add_argument('--force_redo', type=bool, default=True, help="force recompute results")
        ...
        parser.add_argument('--visualize', type=bool, default=True,
                            help='save visualizations when running test')
        parser.add_argument('--average_mode', type=str, default='vote',
                            help='which kind of patch averaging to use for visualizations [vote, before_softmax, '
                                 'after_softmax]')
