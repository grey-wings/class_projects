...
class DeepfakeDataset(Dataset):
    ...
    def __init__(self, normal_root, malicious_root, mode, resize, csv_root):
        self.normal_root = normal_root  # 有普通图片的总文件夹 F:\dataset\faceforensics++\manipulated_sequences
        # \Deepfakes\c23\our_imgs
        self.malicious_root = malicious_root  # 有被修改图片的总文件夹
        self.csv_root = csv_root  # 存放一个csv文件，保存图片和标签
        self.resize = resize  # 修改输入图片大小尺寸
        # self.datalabel= 1 if '_' in str(self.normal_root).split('\\')[-1] else print("error the img has some
        # problem") #加载正常图片 self.datalabel= 0 if '_' in str(self.malicious_root).split('\\')[-1] else print("error
        # the img has some problem") #加载经过了deepfake的图片
