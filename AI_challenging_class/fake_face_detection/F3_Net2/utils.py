...
class FFDataset(data.Dataset):
    ...
    def __init__(self, dataset_root, photo_num=float("inf"), size=299, augment=True):
    ...
    def collect_image(self, root):
        ...
        image_path_list = image_path_list[:min(self.photo_num - 1, len(image_path_list) - 1)]
        image_path_list = [os.path.join(root, i) for i in image_path_list]
        # for i in range(self.photo_num):
        #     if i == len(listdir):
        #         break
        #     split = listdir[i]
        #     split_root = os.path.join(root, split)
        #     image_path_list.append(split_root)
...
def get_dataset(name='train', size=299, root='/data/yike/FF++_std_c40_300frames/', photo_num=300, augment=True):
    ...
    for i in range(total_len):
        ...
        dset = FFDataset(fake, photo_num, size, augment)
        dset.size = size
